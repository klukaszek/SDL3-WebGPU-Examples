...
#ifndef SDL_GPU_SHADERCROSS_WGSL
#define SDL_GPU_SHADERCROSS_WGSL 1
 ...
 * \param createInfo a pointer to an SDL_GPUShaderCreateInfo or
 * SDL_GPUComputePipelineCreateInfo structure depending on whether the shader
 * profile is a compute profile. \param isCompute a flag for whether the shader
 * is a compute shader or not. \returns a compiled SDL_GPUShader or
 * SDL_GPUComputePipeline depending on whether isCompute is set
 *
 * \threadsafety It is safe to call this function from any thread.
 */
extern void *SDL_ShaderCross_CompileFromSPIRV(SDL_GPUDevice *device,
                                              void *createInfo, bool isCompute);
#endif /* SDL_GPU_SHADERCROSS_SPIRVCROSS */
 ...
 * \param createInfo a pointer to an SDL_GPUShaderCreateInfo or
 * SDL_GPUComputePipelineCreateInfo structure depending on whether the shader
 * profile is a compute profile. \param hlslSource the HLSL source code for the
 * shader. \param shaderProfile the shader profile to compile the shader with.
...
/* This is for binary, ANSI-text, or to tell the compiler to try autodetecting
 * UTF using the BOM */
#define DXC_CP_ACP 0
...
static void *SDL_ShaderCross_INTERNAL_CompileDXC(SDL_GPUDevice *device,
                                                 void *createInfo,
                                                 const char *hlslSource,
                                                 const char *shaderProfile,
                                                 UINT encoding, bool spirv) {
  DxcBuffer source;
  IDxcResult *dxcResult;
  IDxcBlob *blob;
  IDxcBlobUtf8 *errors;
  LPCWSTR args[] = {(LPCWSTR)L"-E",
  ...
  /* Non-static DxcInstance, since the functions we call on it are not
   * thread-safe */
  IDxcCompiler3 *dxcInstance = NULL;

  SDL_DxcCreateInstance(&CLSID_DxcCompiler, IID_IDxcCompiler3,
                        (void **)&dxcInstance);
  if (dxcInstance == NULL) {
    return NULL;
  }
  ...
  ret = dxcInstance->lpVtbl->Compile(dxcInstance, &source, args, argCount, NULL,
                                     IID_IDxcResult, (void **)&dxcResult);

  if (ret < 0) {
    SDL_LogError(SDL_LOG_CATEGORY_GPU,
                 "IDxcShaderCompiler3::Compile failed: %X", ret);
  ...
  dxcResult->lpVtbl->GetOutput(dxcResult, DXC_OUT_ERRORS, IID_IDxcBlobUtf8,
                               (void **)&errors, NULL);
  if (errors != NULL && errors->lpVtbl->GetBufferSize(errors) != 0) {
    SDL_LogError(SDL_LOG_CATEGORY_GPU, "HLSL compilation failed: %s",
  ...
  ret = dxcResult->lpVtbl->GetOutput(dxcResult, DXC_OUT_OBJECT, IID_IDxcBlob,
                                     (void **)&blob, NULL);
  if (ret < 0) {
  ...
  if (shaderProfile[0] == 'c' && shaderProfile[1] == 's') {
    ...
    newCreateInfo.format =
        spirv ? SDL_GPU_SHADERFORMAT_SPIRV : SDL_GPU_SHADERFORMAT_DXIL;
...
static void *SDL_ShaderCross_INTERNAL_CompileFXC(SDL_GPUDevice *device,
                                                 void *createInfo,
                                                 const char *hlslSource,
                                                 const char *shaderProfile) {
  ID3DBlob *blob;
  ...
  ret = SDL_D3DCompile(hlslSource, SDL_strlen(hlslSource), NULL, NULL, NULL,
                       ((SDL_GPUShaderCreateInfo *)createInfo)->entrypoint,
                       shaderProfile, 0, 0, &blob, &errorBlob);
...
extern void *SDL_ShaderCross_CompileFromHLSL(SDL_GPUDevice *device,
  ...
  if (format & SDL_GPU_SHADERFORMAT_DXBC) {
    return SDL_ShaderCross_INTERNAL_CompileFXC(device, createInfo, hlslSource,
                                               shaderProfile);
  }
  if (format & SDL_GPU_SHADERFORMAT_DXIL) {
    return SDL_ShaderCross_INTERNAL_CompileDXC(
        device, createInfo, hlslSource, shaderProfile, DXC_CP_ACP, false);
  }
  if (format & SDL_GPU_SHADERFORMAT_SPIRV) {
    return SDL_ShaderCross_INTERNAL_CompileDXC(device, createInfo, hlslSource,
                                               shaderProfile, DXC_CP_ACP, true);
  }

  SDL_SetError(
      "SDL_ShaderCross_CompileFromHLSL: Unexpected SDL_GPUShaderFormat");
  return NULL;
}
...
typedef spvc_result (*pfn_spvc_context_parse_spirv)(spvc_context, const SpvId *,
                                                    size_t, spvc_parsed_ir *);
typedef spvc_result (*pfn_spvc_context_create_compiler)(spvc_context,
                                                        spvc_backend,
                                                        spvc_parsed_ir,
                                                        spvc_capture_mode,
                                                        spvc_compiler *);
typedef spvc_result (*pfn_spvc_compiler_create_compiler_options)(
    spvc_compiler, spvc_compiler_options *);
typedef spvc_result (*pfn_spvc_compiler_options_set_uint)(spvc_compiler_options,
                                                          spvc_compiler_option,
                                                          unsigned);
typedef spvc_result (*pfn_spvc_compiler_install_compiler_options)(
    spvc_compiler, spvc_compiler_options);
...
typedef SpvExecutionModel (*pfn_spvc_compiler_get_execution_model)(
    spvc_compiler compiler);
typedef const char *(*pfn_spvc_compiler_get_cleansed_entry_point_name)(
    spvc_compiler compiler, const char *name, SpvExecutionModel model);
...
static pfn_spvc_compiler_create_compiler_options
    SDL_spvc_compiler_create_compiler_options = NULL;
static pfn_spvc_compiler_options_set_uint SDL_spvc_compiler_options_set_uint =
    NULL;
static pfn_spvc_compiler_install_compiler_options
    SDL_spvc_compiler_install_compiler_options = NULL;
static pfn_spvc_compiler_compile SDL_spvc_compiler_compile = NULL;
static pfn_spvc_context_get_last_error_string
    SDL_spvc_context_get_last_error_string = NULL;
static pfn_spvc_compiler_get_execution_model
    SDL_spvc_compiler_get_execution_model = NULL;
static pfn_spvc_compiler_get_cleansed_entry_point_name
...
#define SDL_spvc_compiler_create_compiler_options                              \
  spvc_compiler_create_compiler_options
#define SDL_spvc_compiler_options_set_uint spvc_compiler_options_set_uint
#define SDL_spvc_compiler_install_compiler_options                             \
  spvc_compiler_install_compiler_options
#define SDL_spvc_compiler_compile spvc_compiler_compile
#define SDL_spvc_context_get_last_error_string                                 \
  spvc_context_get_last_error_string
#define SDL_spvc_compiler_get_execution_model spvc_compiler_get_execution_model
#define SDL_spvc_compiler_get_cleansed_entry_point_name                        \
  spvc_compiler_get_cleansed_entry_point_name

#endif /* SDL_GPU_SHADERCROSS_STATIC */

// For now, we load Tint as a static library and call the exported functions
// since we will only be using the WGSL transpilation when using WASM anyway.
#ifdef SDL_GPU_SHADERCROSS_WGSL /* Tint WGSL */

// Imported from a static tint library.
...
#define SPVC_ERROR(func)                                                       \
  SDL_SetError(#func " failed: %s",                                            \
               SDL_spvc_context_get_last_error_string(context))

void *SDL_ShaderCross_CompileFromWGSL(SDL_GPUDevice *device, void *createInfo,
                                      bool isCompute) {
#if SDL_GPU_SHADERCROSS_WGSL

  SDL_GPUShaderFormat shader_formats = SDL_GetGPUShaderFormats(device);

  // Handle the two cases where we can compile WGSL shaders:
  // 1. WGSL to WGSL (no-op, go straight to SDL_GPUShader)
  // 2. WGSL to SPIR-V (use Tint to transpile WGSL to SPIR-V)
  ...
  if (shader_formats & SDL_GPU_SHADERFORMAT_WGSL) {
    if (isCompute) {
      ...
      return SDL_CreateGPUShader(device, (SDL_GPUShaderCreateInfo *)createInfo);
    }
  } else if (shader_formats & SDL_GPU_SHADERFORMAT_SPIRV) {
    SDL_GPUShaderCreateInfo *currentCreateInfo;
    ...
    uint32_t *spirv = SDL_ShaderCross_INTERNAL_WGSLToSPIRV(
        (const char *)currentCreateInfo->code, currentCreateInfo->code_size,
        &out_size);

    if (!spirv) {
      SDL_SetError(
          "SDL_ShaderCross_CompileFromWGSL: Failed to transpile WGSL to "
          "SPIR-V using Tint.");
      return NULL;
    }

    if (out_size == 0) {
      SDL_SetError("SDL_ShaderCross_CompileFromWGSL: Failed to get size of"
                   " SPIR-V shader using Tint.");
      return NULL;
    }

    SDL_GPUShaderCreateInfo newCreateInfo;
    newCreateInfo = *currentCreateInfo;
    newCreateInfo.code = (const Uint8 *)spirv;
    newCreateInfo.code_size = out_size;
...
void *SDL_ShaderCross_CompileFromSPIRV(SDL_GPUDevice *device,
  ...
  if (shader_formats & SDL_GPU_SHADERFORMAT_SPIRV) {
    if (isCompute) {
      return SDL_CreateGPUComputePipeline(
          device, (SDL_GPUComputePipelineCreateInfo *)originalCreateInfo);
    } else {
      return SDL_CreateGPUShader(device,
                                 (SDL_GPUShaderCreateInfo *)originalCreateInfo);
    }
  ...
  } else if (shader_formats & SDL_GPU_SHADERFORMAT_WGSL) {
#ifdef SDL_GPU_SHADERCROSS_WGSL
    createInfo = (SDL_GPUShaderCreateInfo *)originalCreateInfo;

    // Cross-compile SPIR-V to WGSL using Tint instead of SPIRV-Cross
    char *wgsl = SDL_ShaderCross_INTERNAL_SPIRVToWGSL(createInfo->code,
                                                      createInfo->code_size);
    if (!wgsl) {
      SDL_SetError("SDL_ShaderCross_CompileFromSPIRV: Failed to transpile "
                   "SPIR-V to WGSL using Tint.");
      return NULL;
    } else {
      ...
      if (isCompute) {
        SDL_GPUComputePipelineCreateInfo newCreateInfo;
        newCreateInfo = *(SDL_GPUComputePipelineCreateInfo *)createInfo;
        newCreateInfo.code = (const Uint8 *)wgsl;
        newCreateInfo.code_size = SDL_strlen(wgsl) + 1;
        newCreateInfo.format = SDL_GPU_SHADERFORMAT_WGSL;

        compiledResult =
            (SDL_GPUComputePipelineCreateInfo *)SDL_CreateGPUComputePipeline(
        ...
        newCreateInfo.code_size = SDL_strlen(wgsl) + 1;
        newCreateInfo.format = SDL_GPU_SHADERFORMAT_WGSL;
        compiledResult = (SDL_GPUShaderCreateInfo *)SDL_CreateGPUShader(
            device, &newCreateInfo);
      }
#endif /* SDL_GPU_SHADERCROSS_WGSL */
  ...
  result =
      SDL_spvc_context_parse_spirv(context, (const SpvId *)createInfo->code,
                                   createInfo->code_size / sizeof(SpvId), &ir);
  if (result < 0) {
  ...
  result = SDL_spvc_context_create_compiler(
      context, backend, ir, SPVC_CAPTURE_MODE_TAKE_OWNERSHIP, &compiler);
  if (result < 0) {
  ...
  if (backend == SPVC_BACKEND_HLSL) {
    SDL_spvc_compiler_options_set_uint(
        options, SPVC_COMPILER_OPTION_HLSL_SHADER_MODEL, shadermodel);
    SDL_spvc_compiler_options_set_uint(
        options, SPVC_COMPILER_OPTION_HLSL_NONWRITABLE_UAV_TEXTURE_AS_SRV, 1);
  }
  ...
  cleansed_entrypoint = SDL_spvc_compiler_get_cleansed_entry_point_name(
      compiler, createInfo->entrypoint,
  ...
  if (isCompute) {
    ...
    if (backend == SPVC_BACKEND_HLSL) {
      compiledResult = SDL_ShaderCross_CompileFromHLSL(
          device, &newCreateInfo, translated_source,
    ...
    if (backend == SPVC_BACKEND_HLSL) {
      ...
      compiledResult = SDL_ShaderCross_CompileFromHLSL(
          device, &newCreateInfo, translated_source, profile);
    } else {
      newCreateInfo.code = (const Uint8 *)translated_source;
      newCreateInfo.code_size = SDL_strlen(translated_source) + 1;
...
bool SDL_ShaderCross_Init(void) {
...
#ifndef _GAMING_XBOX
    /* Try to load DXIL, we don't need it directly but if it doesn't exist the
     * code will not be loadable */
    void *dxil_dll = SDL_LoadObject(DXIL_DLL);
    if (dxil_dll == NULL) {
      SDL_LogError(SDL_LOG_CATEGORY_GPU,
                   "Failed to load DXIL library, this will cause pipeline "
                   "creation failures!");
      ...
      SDL_UnloadObject(
          dxil_dll); /* Unload immediately, we don't actually need it */
    }
#endif
  }

  if (dxcompiler_dll != NULL) {
    SDL_DxcCreateInstance = (DxcCreateInstanceProc)SDL_LoadFunction(
        dxcompiler_dll, "DxcCreateInstance");
  ...
  if (d3dcompiler_dll != NULL) {
    SDL_D3DCompile =
        (pfn_D3DCompile)SDL_LoadFunction(d3dcompiler_dll, "D3DCompile");
...
#undef CHECK_FUNC
  ...
  // Load Tint if it's available
#ifdef SDL_GPU_SHADERCROSS_WGSL
  SDL_Log("Loading Tint for WGSL support");
  tint_initialize();
#endif /* SDL_GPU_SHADERCROSS_WGSL */
...
SDL_GPUShaderFormat SDL_ShaderCross_GetSPIRVShaderFormats() {
  ...
  if (
    ...
    // Add support for WGSL if it's available
    supportedFormats |=
        SDL_GPU_SHADERCROSS_WGSL ? SDL_GPU_SHADERFORMAT_WGSL : 0;

#if SDL_GPU_SHADERCROSS_HLSL
    /* SPIRV-Cross + DXC allows us to cross-compile to HLSL, then compile to
     * DXIL */
    if (dxcompiler_dll != NULL) {
      supportedFormats |= SDL_GPU_SHADERFORMAT_DXIL;
    }

    /* SPIRV-Cross + FXC allows us to cross-compile to HLSL, then compile to
     * DXBC */
    if (d3dcompiler_dll != NULL) {

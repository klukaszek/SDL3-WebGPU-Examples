...
static int Init(Context *context) {
  ...
  Uint8 *uploadTransferPtr =
      SDL_MapGPUTransferBuffer(context->Device, uploadTransferBuffer, false);
  SDL_memcpy(uploadTransferPtr, imageData->pixels,
             imageData->w * imageData->h * 4);
  SDL_memcpy(uploadTransferPtr + (imageData->w * imageData->h * 4), bufferData,
             sizeof(bufferData));
  ...
  SDL_UploadToGPUTexture(copyPass,
                         ...
                         &(SDL_GPUTextureRegion){.texture = OriginalTexture,
                                                 .w = imageData->w,
                                                 .h = imageData->h,
                                                 .d = 1},
                         false);
  ...
  SDL_CopyGPUTextureToTexture(
      ...
      &(SDL_GPUTextureLocation){
          .texture = OriginalTexture, .x = 0, .y = 0, .z = 0},
      &(SDL_GPUTextureLocation){.texture = TextureCopy, .x = 0, .y = 0, .z = 0},
      imageData->w, imageData->h, 1, false);
  ...
  SDL_UploadToGPUBuffer(copyPass,
                        ...
                        &(SDL_GPUBufferRegion){.buffer = OriginalBuffer,
                                               .offset = 0,
                                               .size = sizeof(bufferData)},
                        false);
  ...
  SDL_CopyGPUBufferToBuffer(
      copyPass, &(SDL_GPUBufferLocation){.buffer = OriginalBuffer, .offset = 0},
      &(SDL_GPUBufferLocation){.buffer = BufferCopy, .offset = 0},
      sizeof(bufferData), false);

  SDL_EndGPUCopyPass(copyPass);

  // Render the half-size version
  SDL_BlitGPUTexture(cmdbuf,
                     &(SDL_GPUBlitInfo){.source.texture = OriginalTexture,
  ...
  SDL_DownloadFromGPUTexture(
      ...
      &(SDL_GPUTextureRegion){
          .texture = TextureCopy, .w = imageData->w, .h = imageData->h, .d = 1},
      &(SDL_GPUTextureTransferInfo){
  ...
  SDL_DownloadFromGPUBuffer(copyPass,
                            &(SDL_GPUBufferRegion){.buffer = BufferCopy,
                                                   .offset = 0,
                                                   .size = sizeof(bufferData)},
                            &(SDL_GPUTransferBufferLocation){
                                .transfer_buffer = downloadTransferBuffer,
  ...
  if (SDL_memcmp(downloadedData + (imageData->w * imageData->h * 4), bufferData,
                 sizeof(bufferData)) == 0) {
  ...
  if (swapchainTexture != NULL) {
    ...
    SDL_GPURenderPass *clearPass = SDL_BeginGPURenderPass(
        ...
        (SDL_GPUColorTargetInfo[]) {
          {
            .texture = swapchainTexture, .load_op = SDL_GPU_LOADOP_CLEAR,
            .store_op = SDL_GPU_STOREOP_STORE, .clear_color = {0, 0, 0, 1},
            .cycle = false
          }
        },
        1, NULL);
    ...
    SDL_BlitGPUTexture(
        cmdbuf,
        &(SDL_GPUBlitInfo) {
          .source.texture = OriginalTexture, .source.w = TextureWidth,
          .source.h = TextureHeight, .destination.texture = swapchainTexture,
          .destination.w = w / 2, .destination.h = h / 2,
          .load_op = SDL_GPU_LOADOP_LOAD, .filter = SDL_GPU_FILTER_NEAREST
        }
    );
    ...
    SDL_BlitGPUTexture(
        cmdbuf,
        &(SDL_GPUBlitInfo) {
          .source.texture = TextureCopy, .source.w = TextureWidth,
          .source.h = TextureHeight, .destination.texture = swapchainTexture,
          .destination.x = w / 2, .destination.w = w / 2,
          .destination.h = h / 2, .load_op = SDL_GPU_LOADOP_LOAD,
    ...
    SDL_BlitGPUTexture(
        cmdbuf,
        &(SDL_GPUBlitInfo) {
          .source.texture = TextureSmall, .source.w = TextureWidth / 2,
          .source.h = TextureHeight / 2,
          .destination.texture = swapchainTexture, .destination.x = w / 4,
          .destination.y = h / 2, .destination.w = w / 2,
          .destination.h = h / 2, .load_op = SDL_GPU_LOADOP_LOAD,
          .filter = SDL_GPU_FILTER_NEAREST
        }
    );

...
int CommonInit(Context *context, SDL_WindowFlags windowFlags) {
  ...
  SDL_Log("CommonInit: Device created");
  context->Window =
  ...
  SDL_Log("CommonInit: Window created");
  if (!SDL_ClaimWindowForGPUDevice(context->Device, context->Window)) {
    SDL_Log("GPUClaimWindow failed");
    return -1;
  }

  SDL_Log("%s: Window successfully claimed.", context->ExampleName);
  return 0;
}
...
CreateComputePipelineFromShader(SDL_GPUDevice *device,
  ...
  SDL_GPUComputePipeline *pipeline =
      SDL_ShaderCross_CompileFromSPIRV(device, &newCreateInfo, true);
  if (pipeline == NULL) {
...
float *LoadHDRImage(const char *imageFilename, int *pWidth, int *pHeight,
                    int *pChannels, int desiredChannels) {
  char fullPath[256];
  SDL_snprintf(fullPath, sizeof(fullPath), "%sContent/Images/%s", BasePath,
               imageFilename);

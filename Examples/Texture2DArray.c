...
static int Init(Context *context) {
  ...
  SDL_GPUShader *fragmentShader =
      LoadShader(context->Device, "TexturedQuadArray.frag", 1, 0, 0, 0);
  ...
  SDL_memcpy(textureTransferPtr + imageSizeInBytes, imageData2->pixels,
             imageSizeInBytes);
  ...
  SDL_UploadToGPUBuffer(
      copyPass,
      &(SDL_GPUTransferBufferLocation){.transfer_buffer = bufferTransferBuffer,
                                       .offset = 0},
  ...
  SDL_UploadToGPUTexture(
      ...
      &(SDL_GPUTextureRegion){
          .texture = Texture, .w = imageData1->w, .h = imageData1->h, .d = 1},
      false);

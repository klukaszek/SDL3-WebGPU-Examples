...
static void ErrorCallback(WGPUErrorType type, const char *message,
                          void *userdata) {
...
static void MapCallback(WGPUBufferMapAsyncStatus status, void *userdata) {
  if (status == WGPUBufferMapAsyncStatus_Success) {
    SDL_Log("Buffer mapped successfully for writing");
    *(bool *)userdata = true;
  } else {
    SDL_Log("Buffer mapping failed for writing");
  }
}

// Simple barebones WebGPU example to write and read buffer using mapping
// instead of direct data transfer This is then going to be refactored into the
// proper SDL implementation
//
// This code is loosely based on code from Nano WebGPU
void test_WriteAndReadBuffer(WGPUDevice device) {
  // Create a buffer to hold data
  float dataToWrite[BUFFER_SIZE];
  for (size_t i = 0; i < BUFFER_SIZE; ++i) {
    dataToWrite[i] = (float)i; // Fill buffer with sample data
  }

  wgpuDeviceSetUncapturedErrorCallback(device, ErrorCallback, NULL);

  // Create a buffer to write to the GPU (not mapped)
  WGPUBufferDescriptor writeBufferDesc = {
      .usage = WGPUBufferUsage_CopySrc | WGPUBufferUsage_CopyDst,
      .size = sizeof(dataToWrite),
      .mappedAtCreation = false, // Not mapped at creation
  };

  WGPUBuffer writeBuffer = wgpuDeviceCreateBuffer(device, &writeBufferDesc);
  ...
  WGPUBufferDescriptor stagingBufferDesc = {
      ...
      .size = sizeof(dataToWrite),
      .mappedAtCreation = false, // Mapped at creation for upload
  };

  WGPUBuffer stagingBuffer = wgpuDeviceCreateBuffer(device, &stagingBufferDesc);

  bool is_mapped = false;
  wgpuBufferMapAsync(stagingBuffer, WGPUMapMode_Write, 0, sizeof(dataToWrite),
                     MapCallback, &is_mapped);

  // Wait for the staging buffer to be mapped
  while (!is_mapped) {
    emscripten_sleep(1);
    // Busy wait; in practice, you would likely want to implement a proper
    // waiting mechanism
  }

  // Copy data to the mapped staging buffer
  memcpy(wgpuBufferGetMappedRange(stagingBuffer, 0, sizeof(dataToWrite)),
         dataToWrite, sizeof(dataToWrite));
  wgpuBufferUnmap(stagingBuffer); // Unmap after writing

  is_mapped = false;

  // Record commands to copy from the staging buffer to the write buffer
  WGPUCommandEncoder commandEncoder =
      wgpuDeviceCreateCommandEncoder(device, NULL);

  wgpuCommandEncoderCopyBufferToBuffer(commandEncoder, stagingBuffer, 0,
                                       writeBuffer, 0, sizeof(dataToWrite));

  // Finish command buffer
  WGPUCommandBuffer commandBuffer =
      wgpuCommandEncoderFinish(commandEncoder, NULL);
  ...
  WGPUBuffer readBuffer = wgpuDeviceCreateBuffer(device, &readBufferDesc);

  // Record commands to copy data from writeBuffer to readBuffer
  commandEncoder = wgpuDeviceCreateCommandEncoder(device, NULL);
  wgpuCommandEncoderCopyBufferToBuffer(commandEncoder, writeBuffer, 0,
                                       readBuffer, 0, sizeof(dataToWrite));

  // Finish command buffer
  commandBuffer = wgpuCommandEncoderFinish(commandEncoder, NULL);

  // Submit the command buffer to the GPU
  wgpuQueueSubmit(wgpuDeviceGetQueue(device), 1, &commandBuffer);

  // Map the readBuffer for reading
  wgpuBufferMapAsync(readBuffer, WGPUMapMode_Read, 0, sizeof(dataToWrite),
                     MapCallback, &is_mapped);
  ...
  SDL_Log("Reading data from the buffer");

  // Read the data from the mapped buffer
  const float *readData = (const float *)wgpuBufferGetConstMappedRange(
      readBuffer, 0, sizeof(dataToWrite));
  for (size_t i = 0; i < BUFFER_SIZE; ++i) {
    SDL_Log("readData[%zu] = %f\n", i, readData[i]);
  }
  ...
  SDL_Log("Buffer readback test complete");
  SDL_Log("WGPU Successfully Initialized");
}

...
static EM_BOOL emsc_dummy_key_callback(int type, const EmscriptenKeyboardEvent *ev,
                                       void *userdata)
{
...
static EM_BOOL emsc_dummy_mousebutton_callback(int type, const EmscriptenMouseEvent *ev,
                                               void *userdata)
{
    return EM_TRUE;
}

static EM_BOOL emsc_dummy_mousewheel_callback(int type, const EmscriptenWheelEvent *ev,
                                              void *userdata)
{
    return EM_TRUE;
}

static EM_BOOL emsc_dummy_touch_callback(int type, const EmscriptenTouchEvent *ev,
                                         void *userdata)
{
    return EM_TRUE;
}
...
void process_events(Context *context)
{
    ...
    while (SDL_PollEvent(&evt)) {
        ...
        // If we've been processing events for more than 2ms, empty any mouse events in the event queue
        // as there is an issue with mouse events flooding the event queue on browsers.
        if (SDL_GetTicks() - start > 2) {
            SDL_Log("Emptying mouse events from the queue!");
            while (SDL_PeepEvents(&evt, 1, SDL_GETEVENT, SDL_EVENT_MOUSE_MOTION, SDL_EVENT_MOUSE_MOTION) > 0) {
                //
            }
            break;
        }
...
load:
    ...
    if (Examples[exampleIndex]->Init(context) < 0) {
        ...
        if (depth++ < 2)
            goto load;
        SDL_LogError(SDL_LOG_CATEGORY_APPLICATION, "Failed to load fallback example: %s", context->ExampleName);
        return false;
    }
...
int main(int argc, char **argv)
{
    ...
    ctx.Device = SDL_CreateGPUDevice(SDL_ShaderCross_GetSPIRVShaderFormats(), true, NULL);
    if (ctx.Device == NULL) {
    ...
    ctx.Window = SDL_CreateWindow(ctx.ExampleName, 640, 480, SDL_WINDOW_RESIZABLE);
    if (ctx.Window == NULL) {
    ...
    emscripten_set_touchend_callback("#canvas", &ctx, true, emsc_dummy_touch_callback);
